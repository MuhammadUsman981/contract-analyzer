...
# # Default API endpoint (FastAPI backend)
# API_URL = "http://127.0.0.1:8000/analyze/text"
...
#             # Always call API
#             payload = {"text": contract_text}
#             st.info(f"Sending API request to {api_url}")
...
#                      f"Make sure FastAPI backend is running at {api_url}")
#             st.stop()
...
if st.button("🔍 Analyze Contract", type="primary") and contract_text:
    with st.spinner("Analyzing contract with Railway backend..."):
        try:
            payload = {"text": contract_text}
            st.info(f"Sending request to Railway API...")
            ...
            st.error(f"❌ Analysis failed: {str(e)}")
            st.info("Note: This is a demo deployment. Full ML analysis available in development environment.")
...
st.markdown("Backend API: https://contract-analyzer-production-a7ea.up.railway.app")