...
LOCAL_API_URL = "http://localhost:8000/analyze/text"  # This should match backend
RAILWAY_API_URL = "https://contract-analyzer-production-a7ea.up.railway.app/analyze/text"  # Add /text
...
if st.sidebar.button("Test Connection"):
    try:
        health_url = api_url.replace("/analyze/text", "/health")
    ...
    if uploaded_file:
        try:
            if uploaded_file.name.lower().endswith('.pdf'):
                ...
                # Fixed text extraction with accurate page counting
                text_content = []
                actual_pages_with_content = 0
                
                for page_num, page in enumerate(pdf_reader.pages):
                    try:
                        page_text = page.extract_text()
                        if page_text and page_text.strip():  # Only count pages with actual text
                            text_content.append(page_text.strip())
                            actual_pages_with_content += 1
                    except Exception as e:
                        st.warning(f"Could not extract text from page {page_num + 1}: {str(e)}")
                        continue
                
                contract_text = "\n\n".join(text_content)  # Join with double newline
                
                if not contract_text.strip():
                    st.error("Could not extract text from PDF. Please ensure the PDF contains searchable text.")
                else:
                    st.success(f"Successfully extracted text from {actual_pages_with_content} pages")
            else:
